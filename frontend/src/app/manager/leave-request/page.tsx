...
function MyLeaveTab() {
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await leaveRequestService.updateLeaveRequest(editingReq.leaveId, {
        leaveType: editType, startDate: editStart, endDate: editEnd, reason: editReason,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedType || !startDate || !endDate) {
      const finalMsg = 'กรุณาเลือกประเภทการลาและช่วงวันที่';
      setError(finalMsg);
      showToast(finalMsg);
      return;
    }
    ...
    setSubmitting(true);
    try {
      await leaveRequestService.createLeaveRequest({ leaveType: selectedType, startDate, endDate, reason });
      setShowSuccess(true);
      setSelectedType('');
      setStartDate('');
      setEndDate('');
                  ...
                  <div className="flex items-center gap-3 p-3.5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-500 mt-0.5">
                        ...
                        {req.numberOfDays ? <span className="ml-1 font-medium text-gray-700">({req.numberOfDays} วัน)</span> : null}
                      </div>
                      {payText && (
...
function ApproveLeaveTab() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      // Filter out manager's own requests - only show user requests
      const authUserId = authUser?.id ? parseInt(authUser.id, 10) : null;
      const filtered = r.leaveRequests.filter(req => authUserId !== null && req.userId !== authUserId);
...
function ApproveLateTab() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      // Filter out manager's own requests - only show user requests
      const authUserId = authUser?.id ? parseInt(authUser.id, 10) : null;
      const filtered = r.lateRequests.filter(req => authUserId !== null && req.userId !== authUserId);
      setRequests(filtered);
    } catch {
      setMsg('โหลดข้อมูลไม่สำเร็จ');
    } finally {
...
export default function ManagerLeaveRequestPage() {
  return (
    <div className="space-y-4">
      ...
      <Tabs defaultValue="approve-leave" className="w-full">
        ...
        <TabsContent value="approve-leave" className="mt-4"><ApproveLeaveTab /></TabsContent>
        <TabsContent value="approve-late" className="mt-4"><ApproveLateTab /></TabsContent>
        <TabsContent value="my-leave" className="mt-4"><MyLeaveTab /></TabsContent>
        <TabsContent value="my-late" className="mt-4"><MyLateTab /></TabsContent>
      </Tabs>
    </div>
  );
}
