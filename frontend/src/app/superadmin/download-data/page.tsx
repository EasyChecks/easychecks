...
import downloadService, { ReportFormat, PreviewResult } from '@/services/downloadService';
...
export default function DownloadData() {
  ...
  // Load branches — show only up to ภูเก็ต
  const ALLOWED_BRANCHES = ['กรุงเทพ', 'เชียงใหม่', 'ขอนแก่น', 'ภูเก็ต'];
  useEffect(() => {
    dashboardService.getBranchesMap()
      .then(res => {
        const filtered = res.data.filter((b: BranchMapItem) =>
          ALLOWED_BRANCHES.some(city => b.name.includes(city))
        );
  ...
  }, [startDate, endDate, selectedBranchId, validateDates]);
  ...
  }, [reportFormat, startDate, endDate, selectedBranchId, validateDates]);
