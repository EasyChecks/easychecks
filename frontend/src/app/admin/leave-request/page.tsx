...
function MyLeaveTab() {
  ...
  const allowedTypeSet = useMemo(() => new Set(allowedTypeList), [allowedTypeList]);
  ...
  }, [selectedType, allowedTypeSet]);

  const today = new Date().toISOString().split('T')[0];
  const isSick = selectedType === 'SICK';
  const allowPastDaily = selectedType === 'PERSONAL';
  const weekdayCount = (() => {
          ...
          <Card className="w-full max-w-sm max-h-[75vh] p-0 rounded-xl shadow-xl flex flex-col">
            ...
            <div className="p-5 space-y-4 overflow-y-auto flex-1">
              ...
              <div>
                ...
                <textarea value={editReason} onChange={e => setEditReason(e.target.value)}
                  placeholder="บรรยายเหตุผลการมาสาย" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:border-orange-500 focus:outline-none resize-none text-sm" rows={3} />
              </div>
...
function ApproveLeaveTab() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      // Filter out admin's own requests - only show user requests
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Delete Confirmation Modal */}
      {/* This modal should NOT be here - it belongs in MyLeaveTab, not ApproveLeaveTab */}
    </div>
  );
}
